...
独立程序内置自己的 Node 运行时，不读取、切换或覆盖电脑上已有的 Node 14/16/23，也不会修改 nvm 等版本管理器状态。
...
重复打开快捷方式会复用已经运行的本地 UI，不会为每次点击长期保留一个新服务进程。
...
ai-toolops update self [--dry-run|--yes]
...
读取页面或刷新状态不会联网，只有点击“检查更新”才访问 GitHub。
...
```powershell
ai-toolops update self --dry-run
ai-toolops update self --yes
```

它只接受本仓库的 `ai-toolops-setup.exe`，并要求 GitHub Release API 提供 SHA-256 digest。运行中的程序退出后，helper 才调用安装器覆盖升级；安装失败时安装器恢复旧程序。

开发或离线维护仍可手工提供本地/HTTPS 主程序和 SHA-256：
...
Tag 发布工作流只主动上传一个普通用户资产 `ai-toolops-setup.exe`。GitHub 自动显示的两个 Source code 压缩包无法由项目关闭，它们不是额外安装方式。

发布工作流支持可选 Authenticode 证书机密；未配置证书时构建日志会明确标记 `UNSIGNED BUILD`。
